...
/// Greg AI Chat Page - ChatGPT-style UI with streaming
class GregChatPage extends ConsumerStatefulWidget {
  const GregChatPage({super.key});

  @override
  ConsumerState<GregChatPage> createState() => _GregChatPageState();
}

class _GregChatPageState extends ConsumerState<GregChatPage> {
  final ScrollController _scrollController = ScrollController();
  final TextEditingController _messageController = TextEditingController();
  bool _showScrollToBottom = false;

  // Theme State
  GregChatTheme? _forcedTheme;

  // Computed Theme
  GregChatTheme get _currentTheme =>
      _forcedTheme ??
      (Theme.of(context).brightness == Brightness.dark
          ? kGregPredefinedThemes[0]
          : kGregPredefinedThemes[1]);
  ...
  Future<void> _sendMessage(String text, {String? contentType}) async {
    ...
    await ref.read(gregConversationProvider.notifier).sendMessage(text);
    _scrollToBottom();
  }
  ...
  Widget build(BuildContext context) {
    final gregState = ref.watch(gregConversationProvider);
    final gregAccessAsync = ref.watch(gregAccessProvider);

    // Auto-scroll when streaming
    if (gregState.isStreaming) {
        ...
        data: (hasAccess) {
          ...
          return Column(
            children: [
              // Messages
              Expanded(
                child: gregState.messages.isEmpty && !gregState.isStreaming
                    ...
                    : ListView.builder(
                        ...
                        itemCount:
                            gregState.messages.length +
                            (gregState.isStreaming ? 1 : 0),
                        itemBuilder: (context, index) {
                          // Show streaming message
                          if (index == gregState.messages.length) {
                            return _buildStreamingMessage(
                              gregState.streamingMessage ?? '',
                              _currentTheme,
                            );
                          }

                          final msg = gregState.messages[index];
                          ...
                          return _buildWhatsAppMessage(
                            ...
                            onRegenerate:
                                !isUser &&
                                    index == gregState.messages.length - 1 &&
                                    !gregState.isStreaming
                                ? () => ref
                                      .read(gregConversationProvider.notifier)
              ...
              if (gregState.error != null)
                _buildErrorBanner(gregState.error!, _currentTheme),
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context, GregChatTheme theme) {
      ...
      leading: InkWell(
        ...
        child: Row(
          ...
          children: [
            ...
            CircleAvatar(
              radius: 16,
              backgroundImage: gregProfile['image'].startsWith('http')
                  ? NetworkImage(gregProfile['image'])
                  : AssetImage(gregProfile['image']) as ImageProvider,
  ...
  Widget _buildWelcomeScreen(BuildContext context, GregChatTheme theme) {
    final gregProfile = GregAIService.getGregProfile();
    final prompts = GregAIService.getSuggestedPrompts();
        ...
        children: [
          ...
          Center(
            child: Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                border: Border.all(color: const Color(0xFF10A37F), width: 3),
                boxShadow: [
                  BoxShadow(
              ...
              child: ClipRRect(
                ...
                child: gregProfile['image'].startsWith('http')
                    ? Image.network(
                        gregProfile['image'],
                        fit: BoxFit.cover,
                        width: 100,
          ...
          _buildCapabilityItem(
            ...
            'Gestión de Negocio',
            'Ayuda con operaciones y agenda',
            isDark,
          ),
          _buildCapabilityItem(
  ...
  Widget _buildStreamingMessage(String content, GregChatTheme theme) {
    return _buildWhatsAppMessage(
      content: content,
      isUser: false,
      theme: theme,
  ...
  Widget _buildWhatsAppMessage({
    ...
    final isDark = theme.isDark;
    // Parsing logic for video results
  ...
  Widget _buildErrorBanner(String error, GregChatTheme theme) {
    return Container(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            ...
            color: const Color(0xFFEF4444),
            onPressed: () {
              ref.read(gregConversationProvider.notifier).clearError();
            },
          ),
        ],
      ),
  ...
  Widget _buildNoAccessView(BuildContext context, GregChatTheme theme) {
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text(
              'Desbloquea tu asistente de negocio personal',
              style: GoogleFonts.inter(
                fontSize: 16,
                color: theme.secondaryTextColor,
              ),
...
// Video Player with Controls (Robust)
...
final List<GregChatTheme> kGregPredefinedThemes = [

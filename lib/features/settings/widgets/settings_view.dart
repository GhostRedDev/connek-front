...
class _SettingsViewState extends ConsumerState<SettingsView> {
  ...
  Future<void> _updatePhone() async {
    final controller = TextEditingController(
      text: _supabase.auth.currentUser?.phone ?? '',
    );
    await showDialog(
      context: context,
  ...
  Future<void> _deleteAccount() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text("Account Deactivation request sent.")),
    );
  }
                ...
                children: [
                  ...
                  _buildSettingItem(
                    ...
                    onTap: () {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text("Linked: ${providers.join(', ')}"),
                        ),
                      );
                    },
                  ),
                  ...
                  _buildSwitchItem(
                    'Push notifications',
                    pushNotifications,
                    (v) => _updateMetadata('push_notifications', v),
                  ),
                  _buildSwitchItem(
                    'Reminders',
                    reminders,
                    (v) => _updateMetadata('reminders', v),
                  ),
                  _buildSwitchItem(
                    'Messages',
                    messages,
                    (v) => _updateMetadata('messages', v),
                  ),
                  _buildSwitchItem(
                    'Email notifications',
                    emailNotifications,
                    (v) => _updateMetadata('email_notifications', v),
                  ),
                  _buildSwitchItem(
                    'News',
                    news,
                    (v) => _updateMetadata('news', v),
                  ),
                  _buildSwitchItem(
                    'Activity',
                    activity,
                    (v) => _updateMetadata('activity', v),
                  ),
                  _buildSwitchItem(
                    'Reports',
                    reports,
                    (v) => _updateMetadata('reports', v),
                  ),
          ...
          children: [
            if (icon != null) ...[
              Icon(
                icon,
                size: 20,
                color: isDestructive ? Colors.redAccent : Colors.grey[500],
              ),

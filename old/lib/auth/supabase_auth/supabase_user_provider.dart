...
class ConnekSupabaseUser extends BaseAuthUser {
  ConnekSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> connekSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ConnekSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
